...
export class CuccCmpClient {
  ...
  public setDetail: (iccid: string, changeType: string, targetValue: Status | string) => Promise<SetDetailResponse>;
  ...
  private async request(config: AxiosRequestConfig & { retryTimes?: number }): Promise<any> {
    ...
    if (data.resultCode !== '0000') {
      throw new RequestError(`接口业务发生异常！错误码：${data.resultCode}，错误信息：${data.resultDesc}！`, res.config.traceId)
    }
    return data;
  }
  ...
  public static mixin(obj: { [key: string]: Function }): void {
    Object.keys(obj).forEach(key => {
      if (CuccCmpClient.prototype.hasOwnProperty(key)) {
        throw new Error('不允许覆盖已经存在的成员方法 <' + key + '>');
      }
      CuccCmpClient.prototype[key] = obj[key];
    });
  }
}

CuccCmpClient.mixin(apis);

export { Options, CustomOptions, Status };
